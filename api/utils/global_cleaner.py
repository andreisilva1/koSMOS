...
def global_cleaner(df: DataFrame, target: str = None):
    ...
    if len(clean_df) > MAX_ROWS:
        if target and target in clean_df.columns:
            ...
            compressed_df = strat_df.groupby(target, group_keys=False).apply(lambda x: x.sample(max(1, int(MAX_ROWS * len(x) / len(strat_df))), random_state=51))
    ...
    for col in clean_df.columns:
        if pd.api.types.is_object_dtype(clean_df[col]) or pd.api.types.is_string_dtype(clean_df[col]):
            ...
            if clean_df[col].nunique() == 2:
                ...
                clean_df[col] = (clean_df[col] == first_value).astype(int) # If just 2 possible values, transform in a binary column (1 and 0s)
    ...
    # Target validation
    if target:  
        if clean_df[target].nunique() == 1: # Raises a error if the target just had a possible value.
            raise HTTPException(
                status_code=400,
